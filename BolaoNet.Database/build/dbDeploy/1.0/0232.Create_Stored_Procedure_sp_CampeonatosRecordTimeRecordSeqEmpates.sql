IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'sp_CampeonatosRecordTimeRecordSeqEmpates')
BEGIN
	DROP  Procedure  sp_CampeonatosRecordTimeRecordSeqEmpates
END
GO
CREATE PROCEDURE [dbo].[sp_CampeonatosRecordTimeRecordSeqEmpates]
(
	...
	BEGIN
		...
		BEGIN		
			...
			BEGIN
				...
				IF (@CurrentEmpateDentro > @MaxEmpateDentro )
			...
			BEGIN
				...
				IF (@CurrentEmpateFora > @MaxEmpateFora )

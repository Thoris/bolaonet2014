IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'sp_CampeonatosRecordTimeRecordSemGanhar')
BEGIN
	DROP  Procedure  sp_CampeonatosRecordTimeRecordSemGanhar
END
GO
CREATE PROCEDURE [dbo].[sp_CampeonatosRecordTimeRecordSemGanhar]
(
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF (@CurrentDerrota + @CurrentEmpate > @MaxDerrota + @MaxEmpate)
				BEGIN
				...
				IF (@CurrentDerrotaFora + @CurrentEmpateFora > @MaxDerrotaFora + @MaxEmpateFora)
				BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF (@CurrentDerrotaDentro + @CurrentEmpateDentro > @MaxDerrotaDentro + @MaxEmpateDentro)
				BEGIN
		...
		BEGIN		
			...
			IF (@CurrentEmpate + @CurrentDerrota > @MaxEmpate + @MaxDerrota)
			BEGIN
				SET @MaxEmpate = @CurrentEmpate
				SET @MaxDerrota = @CurrentDerrota
			END
			...
			BEGIN
				...
				IF (@CurrentEmpateDentro + @CurrentDerrotaDentro > @MaxEmpateDentro + @MaxDerrotaDentro)
				BEGIN
					SET @MaxEmpateDentro = @CurrentEmpateDentro
					SET @MaxDerrotaDentro = @CurrentDerrotaDentro
				END				
			...
			BEGIN
				...
				IF (@CurrentEmpateFora + @CurrentDerrotaFora > @MaxEmpateFora + @MaxDerrotaFora)
				BEGIN
					SET @MaxEmpateFora = @CurrentEmpateFora
					SET @MaxDerrotaFora = @CurrentDerrotaFora
				END		

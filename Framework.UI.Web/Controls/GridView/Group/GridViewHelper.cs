...
namespace Framework.UI.Web.Controls.GridView.Group
{
    ...
    public class GridViewHelper
    {
        ...
        private System.Web.UI.WebControls.GridView mGrid;
        private List<GridViewSummary> mGeneralSummaries;
        private List<GridViewGroup> mGroups;
        ...
        private const string SUPRESS_GROUP_ALREADY_DEFINED = "A supress group is already defined. You can't define supress AND summary groups simultaneously";
        private const string ONE_GROUP_ALREADY_REGISTERED = "At least a group is already defined. A supress group can't coexist with other groups";
        ...
        public GridViewHelper(System.Web.UI.WebControls.GridView grd) : this(grd, false, SortDirection.Ascending) { }

        public GridViewHelper(System.Web.UI.WebControls.GridView grd, bool useFooterForGeneralSummaries) : this(grd, useFooterForGeneralSummaries, SortDirection.Ascending) { }
        ...
        public GridViewSummary RegisterSummary(string column, SummaryOperation operation)
        {
            ...
            GridViewSummary s = new GridViewSummary(column, operation, null);
        ...
        public GridViewSummary RegisterSummary(string column, SummaryOperation operation, string groupName)
        {
            ...
            GridViewGroup group = FindGroupByName(groupName);
            if (group == null)
            {
            ...
            // TO DO: Perform column validation...
            GridViewSummary s = new GridViewSummary(column, operation, group);
            group.AddSummary(s);
        ...
        {
            // TO DO: Perform column validation...
            GridViewSummary s = new GridViewSummary(column, operation, getResult, null);
            mGeneralSummaries.Add(s);
        ...
        public GridViewSummary RegisterSummary(string column, CustomSummaryOperation operation, SummaryResultMethod getResult, string groupName)
        {
            ...
            // TO DO: Perform column validation...
            GridViewSummary s = new GridViewSummary(column, operation, getResult, group);
            group.AddSummary(s);
        ...
        public GridViewGroup SetSupressGroup(string column)
        {
            string[] cols = new string[1] { column };
            return SetSupressGroup(cols);
        }

        public GridViewGroup SetSupressGroup(string[] columns)
        {
            if (mGroups.Count > 0)
            {
            ...
            // TO DO: Perform column validation...
            GridViewGroup g = new GridViewGroup(columns, true);
            mGroups.Add(g);
        ...
        private bool HasSupressGroup()
        {
            foreach (GridViewGroup g in mGroups)
            {
                if (g.IsSupressGroup) return true;
            }
            return false;
        }
        ...
        private GridViewGroup FindGroupByName(string name)
        {
            foreach (GridViewGroup g in mGroups)
            {
        ...
        private GridViewRow InsertGridRow(GridViewRow beforeRow, GridViewGroup g)
        {
            ...
            if (g != null && (g.IsSupressGroup || g.GenerateAllCellsOnSummaryRow))
            {
        ...
        /// <param name="beforeRow"></param>
        /// <returns></returns>
        private GridViewRow InsertGridRow(GridViewRow beforeRow)
        {
            int visibleColumns = this.GetVisibleColumnCount();
        ...
        private void GenerateGroupSummary(GridViewGroup g, GridViewRow row)
        {
            ...
            if (!HasAutoSummary(g.Summaries) && !HasSupressGroup()) return;
        ...
        private void GenerateGeneralSummaries(GridViewRowEventArgs e)
        {
            ...
            if (!HasAutoSummary(this.mGeneralSummaries)) return;
